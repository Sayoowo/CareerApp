# Retrospectives

## Purpose

Regular team retrospectives to reflect on our process and continuously improve. These are internal team meetings, separate from stakeholder reviews.

**Schedule**: Every 2 weeks throughout the project

**Focus**:
- What went well
- What didn't go well  
- What we learned
- Actions to improve

---

## Retrospective #1: Planning Phase Review

**Date**: November 12, 2025  
**Attendees**: Elaine, Giorgina, Jaiden, Ubi, Sayo  
**Context**: Continuing planning

### What Went Well ✅
- Better communication, e.g., everyone was present in class  
- We all agreed to the team agreement
- Felt good about framework for user stories and acceptance criteria

### What Didn't Go Well ⚠️
- Not sure what to do about what the minimum a user could test would be (iteration 0)
- Next step clear but steps after that not clear
- Assigning tasks is a weak point

### What We Learned 💡
- Need to be really clear about who is doing what
- Need to be realistic about what we can do as an iteration 0

### Action Items 🎯

???

---

## Retrospective #2: Early Development Review

**Date**: November 17, 2025  
**Attendees**: Elaine, Giorgina, Jaiden, Ubi, Sayo  
**Context**: Development stall

### What Went Well ✅
- More clarity/framework for development

### What Didn't Go Well ⚠️
- Everyone was busy and couldn't do a ton of work on the project
- Didn't capture action items to be able to assign to people so confusion there

### What We Learned 💡
- Document action items!

### Action Items 🎯

| Action | Owner | Deadline | Status |
|--------|-------|----------|--------|
| User Story # 6 needs to be refined and divided into subtasks | Team | Today | 🟡 |
| User Story # 5 - complete card | Ubi | Wednesday | 🟡 |
| User Story # 2 - complete card | Jaiden | Wednesday | 🟡 |
| Add/update documentation (e.g., screenshots) | Giorgina | Tuesday | 🟡 |
| User Story # 4 - complete card | Sayo | Wednesday | 🟡 |
| User Story # 3 - complete card | Elaine | Wednesday | 🟡 |

---

## Retrospective #3: Early Development

**Date**: November 26, 2025  
**Attendees**: [Names]  
**Context**: Two weeks into development phase

### What Went Well ✅
- 

### What Didn't Go Well ⚠️
- 

### What We Learned 💡
- 

### Action Items 🎯

| Action | Owner | Deadline | Status |
|--------|-------|----------|--------|
| | | | |

---

## Retrospective #3: Mid-Development

**Date**: December 10, 2025  
**Attendees**: [Names]  
**Context**: Mid-way through feature development

### What Went Well ✅
- 

### What Didn't Go Well ⚠️
- 

### What We Learned 💡
- 

### Action Items 🎯

| Action | Owner | Deadline | Status |
|--------|-------|----------|--------|
| | | | |

---

## Retrospective #4: Near Completion

**Date**: December 24, 2025 (if needed)  
**Attendees**: [Names]  
**Context**: Approaching project completion

### What Went Well ✅
- 

### What Didn't Go Well ⚠️
- 

### What We Learned 💡
- 

### Action Items 🎯

| Action | Owner | Deadline | Status |
|--------|-------|----------|--------|
| | | | |

---

## Final Retrospective: Project Completion

**Date**: Late December/Early January  
**Attendees**: [Names]  
**Context**: Full project reflection

### Project Summary
**What we set out to build**: Job Fair Note App (console application)  
**What we delivered**: [Summary]

### Overall Reflection

**What Went Well (Process)**
- [How did Kanban work for us?]
- [How was team communication?]
- [What practices helped?]

**What Went Well (Technical)**
- [What worked well in our code?]
- [Good technical decisions]
- [Effective tools or approaches]

**Challenges We Faced**
- [Major obstacles]
- [How we addressed them]
- [What we'd do differently]

### Key Learnings

**About Kanban/Agile**
- [What we learned about the methodology]

**About Project Management**
- [Insights about planning, scheduling, communication]

**About Teamwork**
- [What we learned about working as a team]

**Technical Skills**
- [New skills or technologies learned]

### Advice for Future Teams
- [What would we tell another team doing a similar project?]
- [What should they definitely do?]
- [What should they avoid?]

### Team Accomplishments 🎉
- [What are we proud of?]
- [Individual contributions to celebrate]

---

## Patterns Over Time

### How We Improved
- [What got better as project progressed?]

### Recurring Challenges
- [Issues that came up multiple times]

### Process Changes We Made
- [How we adapted our process]

---

## Retrospectives vs. Stakeholder Reviews

**Team Retrospectives** (this document):
- Internal team meetings
- Focus on process improvement
- Bi-weekly schedule: Nov 12, Nov 26, Dec 10, Dec 24 (if needed), Final
- Purpose: Improve how we work together

**Stakeholder Reviews** (with instructor):
- External progress reviews
- Focus on product progress and technical approach
- Schedule: Late November (mid-project), Late December (final)
- Purpose: Get feedback and demonstrate progress

---

**Last Updated**: November 19, 2025
